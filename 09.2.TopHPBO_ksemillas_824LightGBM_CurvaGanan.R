...
#cargo el dataset
dataset <- fread("./datasets/06.2.rank_directo.csv.gz") 
...
ruta  <- "~/buckets/b1/exp/HT8230_rankdirecto2/"
nombre  <- "BO_log.txt"
  ...
  #--> posiblemente esta parte quede afuera de la iteración , no necesita estar en la iteración
    ...
    for( semilla in semillas[1:k] ){
      ...
      if(semilla == semillas[k] && i == 1) {
        ...
        FIname <- paste0('10.2.Top',i,'_sem',semillas[k],'_FImpo.txt')
...
ganame <- paste0('10.2.Top', i, '_', k, 'semillas_pred-Ganan.csv')
fwrite(tb_prediccion, 
       file = ganame, sep = ",")      
}

# Genera CSV con los resultados de única Gananancia para c/u de los
fwrite(resultados, file = '10.2.Resumen_Ganancia_por_modelo.csv', sep = ",")
